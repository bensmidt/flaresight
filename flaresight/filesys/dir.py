...
class Dir(FileSystemObject):
    ...
    def delete_contents_except_asset(
        self,
        path_for_source_media_file_to_keep: str = None,
    ) -> None:
        """
        Deletes the contents of Dir but not Dir itself,
        ...
        Parameters
        ----------
        path_for_source_media_file_to_keep: str
            path of the source media file to keep
        ...
        files = self.get_files()
        for file in files:
            if file.path == path_for_source_media_file_to_keep:
                logging.info(
